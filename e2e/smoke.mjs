...
function makeWavDataUrl() {
  ...
  for (let i = 0; i < n; i++) {
    const t = i / sr
    const v =
      0.6 * Math.sin(2 * Math.PI * 300 * t) + 0.3 * Math.sin(2 * Math.PI * 700 * t)
...
const fftShown = await page
  .locator('dd', { hasText: /^1024$/ })
  .first()
  .isVisible()
  .catch(() => false)
...
const [wavDownload] = await Promise.all([
  ...
  page
    .getByRole('button', { name: 'Download WAV' })
    .waitFor({ timeout: 30000 })
    .then(() => page.getByRole('button', { name: 'Download WAV' }).click()),
])
